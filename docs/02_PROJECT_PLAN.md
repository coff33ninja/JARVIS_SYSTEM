...
- [x] **Spatial mapping (camera → screen zones):** replace fixed-gain cursor mapping with a fitted projective homography (camera frame ↔ virtual desktop, 4-point, DLT, stored in config); add `zone_for()` named regions (per-monitor, left/right/center/edge) and an "active monitor" target so the cursor maps relative to one monitor's rect. See 13_MULTIMONITOR.md. **Status:** done — `CursorMapper.to_screen()` applies the calibrated homography when valid and falls back to anchor+gain otherwise (`app/perception/mapping.py`); `zone_for()` / `point_at_zone()` and active-monitor targeting are wired into `ControlPipeline`; passive RANSAC refinement remains deferred (`control.passive_calibrate` off by default).
- [x] **Auto-calibration ("spatial awareness"):** guided 4-corner pinch calibration — the calibration UI (8766) runs a corner sequence; the pipeline arms pinch capture, each pinch edge records the normalized index tip, and the 4th corner fits + saves the homography live (ADR-012). Passive RANSAC refinement still deferred (`control.passive_calibrate` off by default).
...
- [x] **Dynamic gesture bindings (HUD menu "Gestures" category):** gesture→action mapping moves to a data-driven registry (stable action IDs ↔ gesture conditions), editable live from the fist menu — toggle any gesture level on/off, rebind a gesture to another action, tune thresholds. Registry enforces uniqueness (one action per pose/combination, with an in-menu warning on collisions). This is what makes the setup "dynamic to a point": new gesture variations are bindings, not code. ADR-011. **Status:** done — `ControlPipeline._dispatch` resolves through `GestureRegistry`; Gestures rows toggle per-gesture with a live checkmark; a "Rebind…" two-level picker re-points a gesture at any dispatch action (per-gesture edge re-arm prevents re-fires; collisions keep the picker open with a warning); a Thresholds category tunes each classification threshold (Increase/Decrease/Reset) live into `classify()` and persists on save; registries deep-copy their seed so edits never leak. `attention`/`mode.transfer_toggle` dispatch outside `_dispatch` and are intentionally not listed.
